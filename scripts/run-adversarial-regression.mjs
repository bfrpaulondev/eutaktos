...
const commands = [
  ...
  ['npm', ['test', '--workspace', '@eutaktos/domain', '--', '--run',
    ...
    'src/non-student-assignment.test.ts', 'src/assignment-response.test.ts', 'src/assignment-history.test.ts',
    'src/student-history-queries.test.ts', 'src/notification-delivery.test.ts', 'src/notification-preferences.test.ts',
    'src/scheduling-invariants.test.ts', 'src/manus-hardening.test.ts']],
  ['npm', ['test', '--workspace', '@eutaktos/application', '--', '--run',
    'src/midweek-scheduling-service.test.ts', 'src/organization-service.test.ts', 'src/organization-delete-audit.test.ts',
...
console.log('\nK50 adversarial regression passed.');
