...
-- identical log payload, the call is treated as an already-applied retry and
-- returns without re-applying anything. A different payload under the same
-- migration identity is rejected.
--
...
begin
  ...
  if v_log->>'status' not in ('completed', 'failed') then
    raise exception 'only finished migrations can be durably committed' using errcode = '22023';
  end if;
  ...
  if v_operation_count <> v_change_count then
    raise exception 'migration operations do not match the submitted changes' using errcode = '22023';
  end if;

  -- Validate every change against the log before touching any person row, and
  -- reject duplicate internal ids within the same batch. Update targets carry
  -- their optimistic version captured from fresh authoritative state; creates
  -- must never collide with an existing person id.
  v_seen_ids := '{}';
  for v_i in 1 .. v_change_count loop
    ...
    if jsonb_typeof(v_item) <> 'object'
      ...
      raise exception 'invalid migration change at position %' using errcode = '22023';
    end if;
    v_kind := v_item->>'kind';
    v_entity_id := btrim(coalesce(v_item->>'id', ''));
    if v_entity_id = '' or length(v_entity_id) > 200 then
      raise exception 'invalid migration change identity at position %' using errcode = '22023';
    end if;
    if v_entity_id = any(v_seen_ids) then
      raise exception 'duplicate migration change identity %' using errcode = '22023';
    end if;
    ...
    -- The persisted log records one operation per submitted change, in order.
    if (v_log->'operations' -> (v_i - 1) ->> 'internalId') <> v_entity_id
       or (v_log->'operations' -> (v_i - 1) ->> 'kind') <> v_kind then
      raise exception 'migration operation correlation mismatch' using errcode = '22023';
    end if;
  end loop;

  -- Apply every supported change atomically. Any failure inside this loop
  -- aborts the surrounding transaction, so nothing partial is ever committed.
  for v_i in 1 .. v_change_count loop
    v_item := p_person_changes -> (v_i - 1);
    v_kind := v_item->>'kind';
    ...
    if v_kind = 'create' then
      if exists (
        ...
        raise exception 'duplicate person during migration commit %' using errcode = '22023';
      end if;
      ...
      if jsonb_typeof(coalesce(v_item->'expectedVersion', 'null'::jsonb)) is distinct from 'number' then
        raise exception 'missing expected version for migration update %' using errcode = '22023';
      end if;
  ...
  insert into public.eutaktos_entities (tenant_id, entity_type, entity_id, data)
    values (
      ...
      'hourglass-migration',
      v_migration_id,
      jsonb_build_object('log', v_log, 'rollbackPlan', v_plan, 'postCommitSteps', v_steps)
    );
