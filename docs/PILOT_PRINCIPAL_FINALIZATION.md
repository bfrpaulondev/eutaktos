...
Base main for this finalization pass: `f30ef774da8dcc4ab04313ec956ba8594022c803`
...
| AP2 — Authentication edge cases | **PASS for backend/runtime scope** | Scanner-safe Magic Link works. Permanent server tests cover idle expiry, absolute expiry, revocation, malformed expiry, secure cookie behavior and server-derived capabilities. Supabase Auth logs directly show successful OTP login, repeated OTP requests, provider rate limiting and used/expired one-time tokens returning 403 without logging the token value. Temporary pilot codes are disabled by default. Remaining active-session rotation UI wiring belongs to Manus/frontend. |
...
| AP4 — Multi-user role matrix | **PASS for server/runtime scope** | Production DB disposable role drill already proved admin/limited/ordinary grants and fail-closed identity binding. `api/pilot-role-matrix.test.ts` now makes the server principal matrix permanent and explicitly proves forged browser tenant/actor/capability fields are ignored and same actor IDs remain tenant-scoped. Real three-mailbox login remains an external acceptance item. |
...
The current scanner-safe Eutaktos flow therefore has backend/runtime evidence for both the success and failure paths. Frontend session-rotation wiring remains deliberately outside the principal ownership boundary.
