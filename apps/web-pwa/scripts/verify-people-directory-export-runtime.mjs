...
function connectCdp(url) {
  ...
  socket.addEventListener('message', event => {
    const message = JSON.parse(String(event.data)); const resolver = pending.get(message.id);
...
try {
  ...
  await cdp.send('Page.addScriptToEvaluateOnNewDocument', { source: `(() => {
    ...
    window.fetch = async (input, init) => {
      ...
      if (pathname === '/api/people/directory' && (!init?.method || init.method === 'GET')) return json({
        contractVersion: 'people-directory-v1', generatedAt: '2026-08-25T12:00:00.000Z',
        ...
        people: [{ id: 'person-runtime', displayName: 'Runtime person', preferredLocale: 'pt-PT', active: true, groups: [{ id: 'group-runtime', name: 'Runtime group' }], availability: { status: 'ready', current: 'available', currentReasonCodes: [] }, eligibility: { status: 'ready', enabledAssignmentTypeIds: ['builtin:reading'] }, responsibilities: { status: 'ready', keys: ['service-group-overseer'] }, assignmentHistory: { status: 'ready', lastCompletedMeetingDate: '2026-07-22' } }],
      });
  ...
  const initial = await evaluate(`({ path: location.pathname + location.search, visibleCheckboxes: [...document.querySelectorAll('input[type="checkbox"]')].filter(node => { const rect = node.getBoundingClientRect(); return rect.width > 0 && rect.height > 0; }).length, hasExport: [...document.querySelectorAll('button')].some(node => (node.innerText || node.textContent || '').trim() === 'Exportar'), hasSelectionEntry: [...document.querySelectorAll('button')].some(node => (node.innerText || node.textContent || '').trim() === 'Selecionar pessoas para exportar') })`);
  ...
  const bulkState = await evaluate(`({ url: location.pathname + location.search + location.hash, visibleCheckboxes: [...document.querySelectorAll('input[type="checkbox"]')].filter(node => { const rect = node.getBoundingClientRect(); return rect.width > 0 && rect.height > 0; }).length, exportSelectedDisabled: [...document.querySelectorAll('button')].find(node => (node.innerText || node.textContent || '').trim() === 'Exportar selecionadas')?.disabled ?? null })`);
  ...
  const finalState = await evaluate(`({ url: location.pathname + location.search + location.hash, visibleCheckboxes: [...document.querySelectorAll('input[type="checkbox"]')].filter(node => { const rect = node.getBoundingClientRect(); return rect.width > 0 && rect.height > 0; }).length })`);
