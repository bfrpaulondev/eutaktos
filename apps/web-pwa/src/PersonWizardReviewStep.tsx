...
import type { PersonWizardDraft, PersonWizardMode } from './PersonWizardModel';

export function PersonWizardReviewStep({ mode, locale, draft, initial, households, groups, labels }: { mode: PersonWizardMode; locale: Locale; draft: PersonWizardDraft; initial: PersonWizardDraft; households: readonly HouseholdDto[]; groups: readonly ServiceGroupDto[]; labels: Readonly<{ identity: string; name: string; locale: string; state: string; active: string; inactive: string; organization: string; households: string; groups: string; none: string; eligibility: string; eligible: string; ineligible: string; confirm: string }> }) {
