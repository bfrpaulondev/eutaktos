...
export function ResponsibilitiesSection({ locale, api = responsibilitiesApi }: { locale: Locale; api?: ResponsibilitiesApi }) {
  ...
  const closeCreate = () => { if (saving) return; setOpen(false); setOperationError(null); window.requestAnimationFrame(() => createButtonRef.current?.focus()); };
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setOpen(false); setPersonId(''); setKey(''); setStart(localDate()); setEnd(''); setNotice('assign');
  ...
  return <Box component="section" aria-labelledby="responsibilities-title">
    ...
    <Dialog open={finishingItem !== null} onClose={() => !finishing && setFinishId(null)} fullWidth maxWidth="xs" aria-describedby="responsibility-finish-description"><DialogTitle>{text.confirmTitle}</DialogTitle><DialogContent><Typography id="responsibility-finish-description">{text.confirmBody}</Typography>{finishingItem ? <Typography sx={{ mt: 1 }} fontWeight={700}>{finishingItem.responsibilityKey}</Typography> : null}</DialogContent><DialogActions><Button disabled={finishing} onClick={() => setFinishId(null)}>{text.cancel}</Button><Button variant="contained" disabled={finishing} onClick={() => void finish()}>{finishing ? text.finishing : text.confirm}</Button></DialogActions></Dialog>
  </Box>;
}
