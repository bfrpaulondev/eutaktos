...
import { Alert, Box, Button, Card, CardContent, Chip, Divider, Paper } from '@mui/material';
...
const copy = {
  'pt-PT': { organization: 'Organização', organizationTitle: 'Pessoas e organização', organizationSubtitle: 'Mantém perfis, agregados, grupos, responsabilidades, ausências e permissões no mesmo contexto.', people: 'Pessoas', households: 'Agregados', groups: 'Grupos de serviço', responsibilities: 'Responsabilidades', audit: 'Histórico de auditoria', access: 'Gerir acessos', hourglass: 'Inspecionar export Hourglass', preview: 'Pré-visualização', previewDetail: 'Esta área prepara o fluxo para o Scheduling Core. As ações só aparecem quando existe um percurso funcional disponível.', schedule: 'Agenda', assignments: 'Designações', agendaUnavailableTitle: 'Agenda ainda indisponível', agendaUnavailableDetail: 'As reuniões reais serão apresentadas aqui quando a consulta de agenda estiver disponível. Nenhuma reunião demonstrativa é mostrada.' },
  en: { organization: 'Organization', organizationTitle: 'People and organization', organizationSubtitle: 'Keep profiles, households, groups, responsibilities, absences and permissions in the same context.', people: 'People', households: 'Households', groups: 'Service groups', responsibilities: 'Responsibilities', audit: 'Audit history', access: 'Manage access', hourglass: 'Inspect Hourglass export', preview: 'Preview', previewDetail: 'This area prepares the flow for Scheduling Core. Actions appear only when a functional journey is available.', schedule: 'Agenda', assignments: 'Assignments', agendaUnavailableTitle: 'Agenda is not available yet', agendaUnavailableDetail: 'Real meetings will appear here when the agenda query is available. No demonstration meetings are shown.' },
  es: { organization: 'Organización', organizationTitle: 'Personas y organización', organizationSubtitle: 'Mantén perfiles, grupos familiares, grupos, responsabilidades, ausencias y permisos en el mismo contexto.', people: 'Personas', households: 'Grupos familiares', groups: 'Grupos de servicio', responsibilities: 'Responsabilidades', audit: 'Historial de auditoría', access: 'Gestionar accesos', hourglass: 'Inspeccionar exportación Hourglass', preview: 'Previsualización', previewDetail: 'Esta área prepara el flujo para Scheduling Core. Las acciones aparecen solo cuando existe un recorrido funcional disponible.', schedule: 'Agenda', assignments: 'Asignaciones', agendaUnavailableTitle: 'La Agenda aún no está disponible', agendaUnavailableDetail: 'Las reuniones reales aparecerán aquí cuando esté disponible la consulta de agenda. No se muestran reuniones de demostración.' },
} as const;
...
export function SectionWorkspace({ locale, section }: SectionWorkspaceProps) {
  ...
  return <Box component="section" aria-labelledby={`section-${section}-title`}>
    ...
    {section === 'agenda' ? <Alert severity="info" role="status" aria-live="polite">
      <Typography variant="subtitle2" fontWeight={700}>{text.agendaUnavailableTitle}</Typography>
      <Typography variant="body2">{text.agendaUnavailableDetail}</Typography>
    </Alert> : <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'repeat(2, minmax(0, 1fr))' }, gap: 1.5 }}>
