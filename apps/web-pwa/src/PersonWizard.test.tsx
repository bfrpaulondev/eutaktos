...
function fixture() {
  ...
  const householdsApi: HouseholdsApi = {
    list: vi.fn(async () => households), get: vi.fn(async id => households.find(item => item.id === id)!), create: vi.fn(), delete: vi.fn(),
  ...
  const serviceGroupsApi: ServiceGroupsApi = {
    list: vi.fn(async () => groups), get: vi.fn(async id => groups.find(item => item.id === id)!), create: vi.fn(), delete: vi.fn(),
  ...
  return { calls, apis: { people: peopleApi, households: householdsApi, serviceGroups: serviceGroupsApi, eligibility: eligibilityApi }, seedPeople(value: PersonProfileDto[]) { people = value; } };
}
...
describe('PersonWizard required scenarios', () => {
  ...
  it('11. preserves unauthenticated state', () => { expect(wizardErrorState(new Error('Unauthorized (401)'))).toBe('unauthenticated'); });
  it('12. preserves permission state', () => { expect(wizardErrorState(new Error('Forbidden (403)'))).toBe('permission-error'); });
  it('13. classifies service failure as retryable', () => { expect(wizardErrorState(new Error('People API request failed (503)'))).toBe('retryable-error'); });
