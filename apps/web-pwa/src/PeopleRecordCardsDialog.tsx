...
export function PeopleRecordCardsDialog({ locale, open, onClose }: { locale: Locale; open: boolean; onClose: () => void }) {
  ...
  return <Modal open={open} onCancel={onClose} width={900} title={text.title} destroyOnHidden footer={<Space><Button onClick={onClose}>{text.close}</Button><Button onClick={() => data && printReport(data, locale, text.title)} disabled={state !== 'ready' || !data?.cards.length}>{text.print}</Button></Space>}>
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      <Alert type="info" showIcon title={text.intro} />
      <Space wrap align="end">
        <div><Typography.Text type="secondary">{text.mode}</Typography.Text><br /><Segmented value={mode} options={[{ value: 'year', label: text.year }, { value: 'custom', label: text.custom }]} onChange={value => { cancel(); setData(null); setState('idle'); setMode(value as Mode); }} /></div>
        {mode === 'year' ? <div><Typography.Text type="secondary">{text.year}</Typography.Text><br /><Select aria-label={text.year} value={year} options={years} onChange={value => { cancel(); setData(null); setState('idle'); setYear(value); }} style={{ width: 130 }} /></div> : <>
          <div><Typography.Text type="secondary">{text.from}</Typography.Text><br /><Input aria-label={text.from} type="date" value={from} onChange={event => { cancel(); setData(null); setState('idle'); setFrom(event.target.value); }} /></div>
          <div><Typography.Text type="secondary">{text.to}</Typography.Text><br /><Input aria-label={text.to} type="date" value={to} onChange={event => { cancel(); setData(null); setState('idle'); setTo(event.target.value); }} /></div>
        </>}
        <Button type="primary" onClick={() => void load()} disabled={!request || state === 'loading'}>{text.preview}</Button>
      </Space>
