...
function AppShell({ preferences, setPreferences }: AppShellProps) {
  ...
  const goToTask = (next: TaskNavKey) => {
    ...
    if (`${window.location.pathname}${window.location.search}` !== nextLocation || window.location.hash) {
      window.history.pushState({ task: next }, '', nextLocation);
    }
  ...
  return (
    <Layout style={{ minHeight: '100dvh', background: 'transparent' }}>
      ...
      {desktop ? <Sider
        ...
        style={{ position: 'fixed', insetBlock: 16, insetInlineStart: 16, height: 'calc(100dvh - 32px)', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--ant-color-border-secondary)', background: 'var(--ant-color-bg-container)', zIndex: 10 }}
      >
        ...
        <Menu
          mode="inline"
          selectedKeys={[activeTask]}
          items={menuItems}
          onClick={({ key }) => goToTask(key as TaskNavKey)}
          style={{ borderInlineEnd: 0, paddingInline: 8, background: 'transparent' }}
        />
        <div style={{ position: 'absolute', insetInline: 16, bottom: 16, padding: 12, borderRadius: 12, border: '1px solid var(--ant-color-border-secondary)' }}>
          <AntText type="secondary" style={{ fontSize: 12 }}>{text.privacy}</AntText>
        </div>
      </Sider> : null}
      ...
      <AntDrawer
        ...
        height="auto"
        title={text.more}
        extra={<AntButton type="text" icon={<CloseOutlined />} onClick={closeMore}>{text.close}</AntButton>}
        styles={{ body: { paddingBottom: 'calc(20px + env(safe-area-inset-bottom))' } }}
      >
      ...
      <Content
        id="main"
        tabIndex={-1}
        style={{ marginInlineStart: desktop ? 296 : 0, width: desktop ? 'calc(100% - 296px)' : '100%', maxWidth: 1640, padding: desktop ? '16px 32px 32px' : '12px 12px calc(84px + env(safe-area-inset-bottom))', outline: 'none' }}
      >
