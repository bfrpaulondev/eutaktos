  ...
  return <div className="app-frame">
    ...
    <aside className="glass-rail" aria-label="Navegação principal">
      ...
      <nav className="rail-nav">
        {nav.map(([key, label, icon]) => <button key={key} className={section === key ? 'rail-item is-active' : 'rail-item'} aria-current={section === key ? 'page' : undefined} onClick={() => setSection(key)}><span className="rail-icon" aria-hidden="true">{icon}</span><span>{label}</span></button>)}
      </nav>
    ...
    <main id="main" className="workspace" tabIndex={-1}>
      ...
      <section className="dashboard-grid">
        <article className="glass-surface feature-card assignment-card">
          ...
          <button className="quiet-action">{text.agenda}<span aria-hidden="true">→</span></button>
        </article>
        ...
        <article className="glass-surface feature-card smart-card">
          ...
          <button className="primary-action compact-action">{text.generate}<span aria-hidden="true">↗</span></button>
