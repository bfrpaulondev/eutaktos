...
const response = {
  ...
  people: [{
    ...
    availability: {
      ...
      current: 'available',
      currentReasonCodes: [],
      nextPeriod: { startsAt: '2026-09-01T00:00:00.000Z', endsAt: '2026-09-03T00:00:00.000Z', reasonCode: 'away' },
    },
...
describe('People directory API', () => {
  ...
  it('preserves explicit unavailable subprojections instead of inventing empty facts', () => {
    const parsed = parsePeopleDirectoryResponse({
      ...response,
  ...
  it('uses same-origin GET and preserves HTTP status for auth/error classification', async () => {
    ...
    expect(fetcher).toHaveBeenCalledWith('/api/people/directory', expect.objectContaining({
      method: 'GET',
