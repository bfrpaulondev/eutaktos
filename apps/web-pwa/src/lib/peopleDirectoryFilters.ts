...
export function peopleDirectoryFiltersFromSearch(search: string): PeopleDirectoryFilters {
  ...
  return Object.freeze({ status: status === 'active' || status === 'inactive' ? status : 'all', availability: availability === 'available' || availability === 'unavailable' ? availability : 'all', ...(safeOpaqueFilter(params.get(urlKeys.group)) ? { groupId: safeOpaqueFilter(params.get(urlKeys.group)) } : {}), ...(safeOpaqueFilter(params.get(urlKeys.eligibility)) ? { eligibilityTypeId: safeOpaqueFilter(params.get(urlKeys.eligibility)) } : {}), ...(safeOpaqueFilter(params.get(urlKeys.responsibility)) ? { responsibilityKey: safeOpaqueFilter(params.get(urlKeys.responsibility)) } : {}) });
}
...
export function peopleDirectorySearchWithFilters(currentSearch: string, filters: PeopleDirectoryFilters): string {
  ...
  if (filters.status !== 'all') params.set(urlKeys.status, filters.status); if (filters.availability !== 'all') params.set(urlKeys.availability, filters.availability); if (filters.groupId) params.set(urlKeys.group, filters.groupId); if (filters.eligibilityTypeId) params.set(urlKeys.eligibility, filters.eligibilityTypeId); if (filters.responsibilityKey) params.set(urlKeys.responsibility, filters.responsibilityKey);
...
export function sanitizePeopleDirectoryFilters(filters: PeopleDirectoryFilters, directory: PeopleDirectoryDto): PeopleDirectoryFilters {
  const validGroupIds = new Set(directory.filters.groups.map(group => group.id)); const validEligibility = new Set(directory.filters.assignmentTypeIds); const validResponsibilities = new Set(directory.filters.responsibilityKeys); const validLabels = new Set(directory.filters.labels ?? []);
  const sanitized = Object.freeze({ status: filters.status, availability: directory.capabilities.availability ? filters.availability : 'all', ...(filters.groupId && validGroupIds.has(filters.groupId) ? { groupId: filters.groupId } : {}), ...(directory.capabilities.eligibility && filters.eligibilityTypeId && validEligibility.has(filters.eligibilityTypeId) ? { eligibilityTypeId: filters.eligibilityTypeId } : {}), ...(directory.capabilities.responsibilities && filters.responsibilityKey && validResponsibilities.has(filters.responsibilityKey) ? { responsibilityKey: filters.responsibilityKey } : {}), ...(filters.label && validLabels.has(filters.label) ? { label: filters.label } : {}) });
...
export function filterPeopleDirectory(people: readonly PeopleDirectoryPersonDto[], query: string, filters: PeopleDirectoryFilters, locale: Locale): readonly PeopleDirectoryPersonDto[] {
  ...
  return people.filter(person => {
    ...
    if (filters.status === 'active' && !person.active) return false; if (filters.status === 'inactive' && person.active) return false;
...
export function hasPeopleDirectoryFilters(query: string, filters: PeopleDirectoryFilters): boolean { return Boolean(query.trim()) || filters.status !== 'all' || filters.availability !== 'all' || Boolean(filters.groupId || filters.eligibilityTypeId || filters.responsibilityKey || filters.label); }
