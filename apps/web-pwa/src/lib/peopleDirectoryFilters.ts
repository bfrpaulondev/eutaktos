...
const urlKeys = Object.freeze({
  status: 'pf_status',
  availability: 'pf_availability',
  group: 'pf_group',
  eligibility: 'pf_eligibility',
  responsibility: 'pf_responsibility',
});

function safeOpaqueFilter(value: string | null): string | undefined {
...
export function peopleDirectoryFiltersFromSearch(search: string): PeopleDirectoryFilters {
  ...
  const status = params.get(urlKeys.status);
  const availability = params.get(urlKeys.availability);
...
export function peopleDirectorySearchWithFilters(currentSearch: string, filters: PeopleDirectoryFilters): string {
  ...
  Object.values(urlKeys).forEach(key => params.delete(key));
...
export function sanitizePeopleDirectoryFilters(filters: PeopleDirectoryFilters, directory: PeopleDirectoryDto): PeopleDirectoryFilters {
  ...
  const sanitized = Object.freeze({
    ...
    ...(directory.capabilities.eligibility && filters.eligibilityTypeId && validEligibility.has(filters.eligibilityTypeId)
      ? { eligibilityTypeId: filters.eligibilityTypeId }
      : {}),
...
export function filterPeopleDirectory(
  ...
  locale: Locale,
): readonly PeopleDirectoryPersonDto[] {
  ...
  return people.filter(person => {
    ...
    if (filters.eligibilityTypeId) {
      if (person.eligibility.status !== 'ready' || !person.eligibility.enabledAssignmentTypeIds.includes(filters.eligibilityTypeId)) return false;
    }
    if (filters.responsibilityKey) {
      if (person.responsibilities.status !== 'ready' || !person.responsibilities.keys.includes(filters.responsibilityKey)) return false;
    }
    return true;
  });
}
