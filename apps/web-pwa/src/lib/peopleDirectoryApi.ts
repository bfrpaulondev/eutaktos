...
function availability(value: unknown): DirectoryAvailability {
  const item = record(value); if (item.status === 'unavailable') return Object.freeze({ status: 'unavailable' });
  if (item.status !== 'ready' || (item.current !== 'available' && item.current !== 'unavailable') || !Array.isArray(item.currentReasonCodes)) throw new Error('Invalid People directory response');
  ...
  return Object.freeze({ status: 'ready', current: item.current, currentReasonCodes: Object.freeze(item.currentReasonCodes as ('away' | 'unavailable' | 'other')[]), ...(nextPeriod ? { nextPeriod } : {}) });
}
