...
const valid = {
  ...
  excluded: [{
    ...
    status: 'excluded',
    reasons: [{ code: 'NOT_ELIGIBLE' }],
    warnings: [{ code: 'NO_COMPLETED_ASSIGNMENT_HISTORY' }],
    history: { kind: 'no-completed-history' },
...
describe('C5.5 People recommendation API client', () => {
  it('parses the reviewed PX7 response without changing reason, warning or rank order', () => {
    ...
    expect(parsed.excluded[0]?.reasons.map(item => item.code)).toEqual(['NOT_ELIGIBLE']);
  });
