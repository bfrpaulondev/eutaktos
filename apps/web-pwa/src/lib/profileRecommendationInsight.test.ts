...
function overview(): MidweekOverviewDto {
  return Object.freeze({
    meetings: Object.freeze([
      Object.freeze({
        id: 'meeting-past', date: '2032-05-20', localTime: '19:30', timezone: 'Europe/Lisbon', state: 'published' as const,
        slots: Object.freeze([{ id: 'slot-past', position: 0, durationMinutes: 5, titleKey: 'past', partDefinitionId: 'builtin:apply-yourself-to-the-ministry' }]),
      }),
      Object.freeze({
        id: 'meeting-1', date: '2032-06-10', localTime: '19:30', timezone: 'Europe/Lisbon', state: 'draft' as const,
        slots: Object.freeze([
      ...
      Object.freeze({
        id: 'meeting-2', date: '2032-06-17', localTime: '19:30', timezone: 'Europe/Lisbon', state: 'published' as const,
        slots: Object.freeze([
      ...
      Object.freeze({
        id: 'meeting-cancelled', date: '2032-06-24', localTime: '19:30', timezone: 'Europe/Lisbon', state: 'cancelled' as const,
        slots: Object.freeze([{ id: 'slot-cancelled', position: 0, durationMinutes: 5, titleKey: 'cancelled', partDefinitionId: 'builtin:apply-yourself-to-the-ministry' }]),
      }),
    ]),
    studentAssignments: Object.freeze([
      Object.freeze({ id: 'assignment-occupied', meetingId: 'meeting-1', slotId: 'slot-occupied', studentId: 'person-other', studentDisplayName: 'Other', assistantId: null, assistantDisplayName: null, state: 'assigned' as const }),
    ]),
...
function recommendation(meetingId: string, slotId: string, personId: string | undefined, rank = 2): PeopleRecommendationDto {
  return Object.freeze({
    ...
    target: Object.freeze({
      ...
      meetingDate: meetingId === 'meeting-1' ? '2032-06-10' : '2032-06-17',
      startsAt: '2032-06-10T18:30:00.000Z',
      endsAt: '2032-06-10T18:35:00.000Z',
    }),
    candidates: Object.freeze(personId ? [Object.freeze({
      personId,
      displayName: 'Profile Person',
      status: 'candidate' as const,
      rank,
      reasons: Object.freeze([{ code: 'ELIGIBLE' as const }, { code: 'AVAILABLE' as const }, { code: 'NO_MEETING_CONFLICT' as const }]),
...
function dependencies(get: ProfileRecommendationInsightDependencies['recommendations']['get'], capabilities = ['people.read', 'eligibility.read', 'availability.read', 'schedule.read']): ProfileRecommendationInsightDependencies {
  return Object.freeze({
    session: { current: vi.fn(async () => Object.freeze({ actorId: 'actor-1', capabilities: Object.freeze(capabilities) }) as never) },
...
describe('C5.7 profile recommendation insight', () => {
  ...
  it('returns only approved positive PX7 candidate evidence without recalculating rank', async () => {
    ...
    if (result.status !== 'ready') throw new Error('Expected ready insight');
    expect(result.partial).toBe(false);
    expect(result.insights).toHaveLength(1);
    expect(result.insights[0]).toMatchObject({ rank: 3, target: { meetingId: 'meeting-2', slotId: 'slot-2' } });
  ...
  it('keeps uncertainty explicit when a target fails and there is no positive evidence', async () => {
    const get = vi.fn(async (meetingId: string, slotId: string) => {
      if (meetingId === 'meeting-2') throw new Error('People recommendation request failed (503)');
      return recommendation(meetingId, slotId, undefined);
    });
  ...
  it('may show confirmed positive evidence while flagging another target as partial', async () => {
    const get = vi.fn(async (meetingId: string, slotId: string) => {
      if (meetingId === 'meeting-2') throw new Error('People recommendation request failed (503)');
      return recommendation(meetingId, slotId, 'person-1', 1);
    });
    const result = await loadProfileRecommendationInsight('person-1', new Date('2032-06-01T12:00:00.000Z'), undefined, dependencies(get as never));
    expect(result.status).toBe('ready');
    if (result.status !== 'ready') throw new Error('Expected ready insight');
