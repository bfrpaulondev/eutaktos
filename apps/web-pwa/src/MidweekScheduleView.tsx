...
const copy = {
  'pt-PT': {
    ...
    cancelConfirm: 'Tem a certeza que pretende cancelar esta designação? O histórico será mantido.', publish: 'Publicar programação', publishConfirm: 'Publicar a programação?', publishBlocked: 'Existem vagas ou conflitos. Resolva-os antes de publicar.',
  ...
  en: {
    ...
    cancelConfirm: 'Cancel this assignment? Its history will be kept.', publish: 'Publish schedule', publishConfirm: 'Publish the schedule?', publishBlocked: 'There are vacancies or conflicts. Resolve them before publishing.',
  ...
  es: {
    ...
    cancelConfirm: '¿Cancelar esta asignación? Se mantendrá su historial.', publish: 'Publicar programación', publishConfirm: '¿Publicar la programación?', publishBlocked: 'Hay vacantes o conflictos. Resuélvalos antes de publicar.',
...
function dateLabel(date: string, localTime: string, locale: Locale): string {
  ...
  const formatted = Number.isFinite(instant.getTime()) ? new Intl.DateTimeFormat(language, { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(instant) : date;
...
export function MidweekScheduleView({ meetingId, locale, canWrite, onChanged, onBack }: { meetingId: string; locale: Locale; canWrite: boolean; onChanged?: () => void | Promise<void>; onBack?: () => void }) {
  ...
  const loadCandidates = useCallback(async (slotId: string, role: CandidateRole, assignmentTypeId?: string, excludePersonIds?: readonly string[]) => {
    const version = ++candidateVersionRef.current;
    setCandidatesLoading(true); setCandidatesError(false); setCandidates(null);
    try {
      const next = await midweekApi.candidates(meetingId, { slotId, role, ...(assignmentTypeId ? { assignmentTypeId } : {}), ...(excludePersonIds?.length ? { excludePersonIds } : {}) });
      if (version === candidateVersionRef.current) setCandidates(next);
    } catch { if (version === candidateVersionRef.current) setCandidatesError(true); }
    finally { if (version === candidateVersionRef.current) setCandidatesLoading(false); }
  }, [meetingId]);

  const closeDrawer = () => { candidateVersionRef.current += 1; setDrawer(CLOSED_DRAWER); setCandidates(null); setCandidatesError(false); };
  const refreshAfterMutation = async () => { closeDrawer(); await load(); await onChanged?.(); };
  const runMutation = async (operation: () => Promise<unknown>, fallback: string) => {
    if (actionInFlight) return false;
    setActionInFlight(true); setActionError(null);
    try { await operation(); await refreshAfterMutation(); return true; }
    catch (error) { setActionError(error instanceof Error ? error.message : fallback); return false; }
  ...
  const openStudentAssign = (slot: ScheduleSlotViewDto) => {
    ...
    setActionError(null);
    setDrawer({ open: true, mode: 'assign', slotId: slot.slotId, role: 'student', titleKey: slot.titleKey, assignmentTypeId: slot.partDefinitionId, assistantRequirement: part?.assistantRequirement ?? 'none' });
  ...
  const openStudentReplace = (slot: ScheduleSlotViewDto) => {
    ...
    setActionError(null);
    setDrawer({ open: true, mode: 'replace', slotId: slot.slotId, role: 'student', titleKey: slot.titleKey, assignmentId: slot.studentAssignmentId, assignmentTypeId: slot.partDefinitionId, excludePersonIds: excludes, preservedAssistantId: slot.assistantId });
    void loadCandidates(slot.slotId, 'student', undefined, excludes);
  };

  const resolvedRoleForSlot = (slotId: string): string => resolveAssignmentTypeChoice(roleChoiceBySlot[slotId] ?? '', customRoleBySlot[slotId] ?? '');

  const roleForSlot = (slot: ScheduleSlotViewDto): string => {
    const definedPart = builtinPart(slot.partDefinitionId);
    if (definedPart && !definedPart.studentNeeded && slot.partDefinitionId) return slot.partDefinitionId;
    return resolvedRoleForSlot(slot.slotId);
  };

  const openNonStudentAssign = (slot: ScheduleSlotViewDto) => {
    const role = roleForSlot(slot);
    if (!role) { setActionError(text.chooseRole); return; }
    setActionError(null);
  ...
  const completeStudentAssignment = async (studentId: string, assistantId?: string | null) => {
    await runMutation(() => midweekApi.assignStudent(meetingId, { slotId: drawer.slotId, studentId, assistantId: assistantId ?? null }), text.assignError);
  };
  ...
  const selectCandidate = async (candidate: CandidateProfileDto) => {
    ...
    if (drawer.mode === 'replace') {
      ...
      if (drawer.role === 'student') await runMutation(() => midweekApi.replaceStudent(drawer.assignmentId!, { studentId: candidate.personId, assistantId: drawer.preservedAssistantId ?? null }), text.assignError);
      else if (drawer.role === 'non-student') await runMutation(() => midweekApi.replaceNonStudent(drawer.assignmentId!, candidate.personId), text.assignError);
      return;
    }
    if (drawer.role === 'non-student') {
      if (!drawer.assignmentTypeId) return;
      await runMutation(() => midweekApi.assignNonStudent(meetingId, { slotId: drawer.slotId, personId: candidate.personId, role: drawer.assignmentTypeId! }), text.assignError);
      return;
    }
    ...
    if (requirement === 'none') { await completeStudentAssignment(candidate.personId, null); return; }
    setDrawer({ ...drawer, role: 'assistant', pendingStudentId: candidate.personId, excludePersonIds: [candidate.personId] });
  ...
  const cancelAssignment = async (assignmentId: string, student: boolean) => {
    ...
    await runMutation(() => student ? midweekApi.cancelStudent(assignmentId) : midweekApi.cancelNonStudent(assignmentId), text.cancelError);
  };

  const publishMeeting = async () => {
    if (!view) return;
    if (view.vacantSlots > 0 || view.conflictedSlots > 0) { setActionError(text.publishBlocked); return; }
  ...
  if (loading) return <section aria-busy="true" role="status" aria-live="polite" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, paddingBlock: 56 }}><Spin size="small" /><Typography.Text type="secondary">{text.loading}</Typography.Text></section>;
  if (loadError) return <Alert type="warning" showIcon title={text.error} action={<Button size="small" onClick={() => void load()}>{text.retry}</Button>} />;
  if (!view) return null;
  ...
  return <section aria-labelledby={`schedule-view-${meetingId}`}>
    <Space direction="vertical" size="large" style={{ display: 'flex' }}>
      <Card><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        <div><Typography.Title level={2} id={`schedule-view-${meetingId}`} style={{ marginBlock: '0 4px' }}>{dateLabel(view.date, view.localTime, locale)}</Typography.Title><Typography.Text type="secondary">{view.timezone}</Typography.Text><div style={{ marginTop: 8 }}><Tag color={view.state === 'published' ? 'success' : view.state === 'cancelled' ? 'warning' : undefined}>{stateLabel(view.state, text)}</Tag></div></div>
        <Space wrap>{onBack ? <Button onClick={onBack}>{text.back}</Button> : null}<Button onClick={() => void load()}>{text.refresh}</Button>{canPublish ? <Button type="primary" loading={actionInFlight} onClick={() => void publishMeeting()}>{text.publish}</Button> : null}</Space>
      </div></Card>

      <Card><Space wrap size="large"><StatisticPill label={text.filled} value={view.filledSlots} total={view.totalSlots} tone="success" /><StatisticPill label={text.vacant} value={view.vacantSlots} total={view.totalSlots} tone="warning" />{view.conflictedSlots > 0 ? <StatisticPill label={text.conflicts} value={view.conflictedSlots} total={view.totalSlots} tone="error" /> : null}<Typography.Text type="secondary">{view.filledSlots}/{view.totalSlots} {text.designations}</Typography.Text></Space></Card>
      ...
      {view.slots.length === 0 ? <Card><Empty description={text.noSlots} /></Card> : <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
        {view.slots.map(slot => {
          ...
          const hasDefinedNonStudentPart = Boolean(slot.partDefinitionId && part && !part.studentNeeded);
          const roleChoice = roleChoiceBySlot[slot.slotId] ?? '';
          return <Card key={slot.slotId}><Space direction="vertical" size="small" style={{ display: 'flex' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div><Typography.Title level={5} style={{ marginBlock: 0 }}>{slot.position + 1}. {part?.label[locale] ?? slot.titleKey}</Typography.Title><Typography.Text type="secondary">{slot.durationMinutes} {text.minutes}</Typography.Text></div><Tag color={slot.state === 'filled' ? 'success' : slot.state === 'conflict' ? 'error' : 'warning'}>{slotStateLabel(slot.state, text)}</Tag></div>

            {slot.studentDisplayName ? <><Typography.Text><strong>{text.student}:</strong> {slot.studentDisplayName}</Typography.Text><Typography.Text><strong>{text.assistant}:</strong> {slot.assistantDisplayName ?? text.noAssistant}</Typography.Text>{canWrite && slot.studentAssignmentId ? <Space wrap><Button size="small" disabled={actionInFlight} onClick={() => openStudentReplace(slot)}>{text.replace}</Button><Button size="small" danger disabled={actionInFlight} onClick={() => void cancelAssignment(slot.studentAssignmentId!, true)}>{text.cancel}</Button></Space> : null}</> : null}

            {slot.nonStudentDisplayName ? <><Typography.Text><strong>{text.role}:</strong> {part?.label[locale] ?? slot.nonStudentRole}</Typography.Text><Typography.Text>{slot.nonStudentDisplayName}</Typography.Text>{canWrite && slot.nonStudentAssignmentId ? <Space wrap><Button size="small" disabled={actionInFlight} onClick={() => openNonStudentReplace(slot)}>{text.replace}</Button><Button size="small" danger disabled={actionInFlight} onClick={() => void cancelAssignment(slot.nonStudentAssignmentId!, false)}>{text.cancel}</Button></Space> : null}</> : null}
            ...
            {!slot.studentDisplayName && !slot.nonStudentDisplayName && canWrite ? <Space direction="vertical" size="small" style={{ display: 'flex' }}>
              ...
              {hasDefinedNonStudentPart ? <Button type="primary" size="small" disabled={actionInFlight} onClick={() => openNonStudentAssign(slot)}>{text.assignRole}</Button> : null}
              {!canAssignStudent && !hasDefinedNonStudentPart ? <><Select aria-label={text.chooseRole} placeholder={text.chooseRole} value={roleChoice || undefined} onChange={value => setRoleChoiceBySlot(current => ({ ...current, [slot.slotId]: value }))} options={[...STANDARD_NON_STUDENT_ROLES.map(option => ({ value: option.id, label: option.label[locale] })), { value: CUSTOM_ASSIGNMENT_TYPE_CHOICE, label: text.customRole }]} style={{ width: 'min(100%, 360px)' }} />{roleChoice === CUSTOM_ASSIGNMENT_TYPE_CHOICE ? <Input aria-label={text.customRoleId} placeholder={text.customRoleId} value={customRoleBySlot[slot.slotId] ?? ''} onChange={event => setCustomRoleBySlot(current => ({ ...current, [slot.slotId]: event.target.value }))} maxLength={200} /> : null}<Button type="primary" size="small" disabled={actionInFlight || !resolvedRoleForSlot(slot.slotId)} onClick={() => openNonStudentAssign(slot)}>{text.assignRole}</Button></> : null}
            </Space> : null}
          </Space></Card>;
        })}
      </Space>}
    ...
    <Drawer title={`${text.candidatesFor}: ${drawer.titleKey}`} open={drawer.open} onClose={closeDrawer} width={Math.min(480, typeof window !== 'undefined' ? window.innerWidth - 32 : 480)} destroyOnClose aria-label={text.candidates}>
...
function CandidatesPanel({ candidates, loading, error, text, onSelect, inFlight }: { candidates: readonly CandidateProfileDto[]; loading: boolean; error: boolean; text: Copy; onSelect: (candidate: CandidateProfileDto) => void; inFlight: boolean }) {
  const [search, setSearch] = useState('');
  const filtered = useMemo(() => { const query = search.trim().toLocaleLowerCase(); return query ? candidates.filter(candidate => candidate.displayName.toLocaleLowerCase().includes(query)) : candidates; }, [candidates, search]);
  ...
  return <Space direction="vertical" size="middle" style={{ display: 'flex' }}><Input.Search allowClear value={search} onChange={event => setSearch(event.target.value)} placeholder={text.search} aria-label={text.search} />{filtered.map(candidate => {
    const selectable = validCandidate(candidate);
    return <Card key={candidate.personId} size="small"><Space direction="vertical" size="small" style={{ display: 'flex' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' }}><Typography.Text strong>{candidate.displayName}</Typography.Text>{!candidate.eligible ? <Tag color="warning">{text.ineligible}</Tag> : candidate.conflicts.length > 0 || !candidate.available ? <Tag color="error">{text.unavailable}</Tag> : <Tag color="success">OK</Tag>}</div><Typography.Text type="secondary">{candidateHint(candidate, text)}</Typography.Text>{selectable ? <Button type="primary" size="small" disabled={inFlight} onClick={() => onSelect(candidate)}>{text.select}</Button> : null}</Space></Card>;
  })}</Space>;
}
