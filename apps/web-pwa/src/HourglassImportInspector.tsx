...
export function HourglassImportInspector({ locale, open, onClose }: { locale: Locale; open: boolean; onClose: () => void }) {
  ...
  const busy = loading || previewState === 'loading' || executeState === 'preparing' || executeState === 'executing';
  const close = () => {
    if (busy) return;
    cancelPreview();
    setInspection(null);
  ...
  return <Modal open={open} onCancel={close} width={760} title={text.title} footer={<Button onClick={close} disabled={busy}>{text.close}</Button>} destroyOnHidden>
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      {inspection ? <Card title={<Space wrap><span>{sourceLabel}</span><Tag>{format}</Tag></Space>} extra={<Button type="link" onClick={() => resetInspection()} disabled={busy}>{text.reset}</Button>}>
        {isHourglassJsonInspection(inspection) ? <JsonSummary inspection={inspection} text={text} onCompare={() => void compare()} onPrepare={() => void prepare()} onExecute={confirmExecute} previewState={previewState} preview={preview} executeState={executeState} prepared={prepared} result={result} /> : null}
...
function JsonSummary({ inspection, text, onCompare, onPrepare, onExecute, previewState, preview, executeState, prepared, result }: { inspection: Readonly<HourglassImportInspection>; text: InspectorCopy; onCompare: () => void; onPrepare: () => void; onExecute: () => void; previewState: PreviewState; preview: HourglassPreviewDto | null; executeState: ExecuteState; prepared: HourglassPreparedExecutionDto | null; result: HourglassExecutionResultDto | null }) {
  const busy = previewState === 'loading' || executeState === 'preparing' || executeState === 'executing';
