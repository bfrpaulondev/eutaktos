...
export function sanitizeHourglassFilename(filename: string): string {
...
export function HourglassImportInspector({ locale, open, onClose }: { locale: Locale; open: boolean; onClose: () => void }) {
  ...
  const onFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const safeFilename = sanitizeHourglassFilename(file.name);
    if (!safeFilename) { setError('unsafe'); return; }
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card size="small" title={text.source}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          <label htmlFor="hourglass-import-file" style={visuallyHiddenFileInput}>{text.choose}</label>
          <input id="hourglass-import-file" ref={inputRef} type="file" accept={source === 'json' ? 'application/json,.json' : 'text/csv,.csv'} aria-describedby="hourglass-import-file-help" style={visuallyHiddenFileInput} onChange={event => void onFile(event)} />
          <Button type="primary" aria-controls="hourglass-import-file" onClick={() => inputRef.current?.click()} disabled={busy} block>{text.choose}</Button>
          <Text id="hourglass-import-file-help" type="secondary">{text.fileHelp}</Text>
          {selectedFilename ? <Space wrap><Text aria-live="polite">{text.selectedFile}: {selectedFilename}</Text><Button size="small" onClick={() => resetInspection()} disabled={busy}>{text.clearFile}</Button></Space> : null}
        </Space>
      </Card>
