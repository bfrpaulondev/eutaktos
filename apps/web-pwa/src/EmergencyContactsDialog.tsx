...
export function EmergencyContactsDialog({ personId, personName, locale, open, onClose }: { personId: string; personName: string; locale: Locale; open: boolean; onClose(): void }) {
  ...
  const closeDialog = () => {
    ...
    if (hasUnsavedEmergencyContactDraft(name, phone, relationship)) {
      setDiscardOpen(true);
      return;
    }
    onClose();
  ...
  return <Dialog open={open} onClose={closeDialog} fullWidth maxWidth="sm" aria-labelledby="emergency-contacts-title">
    ...
    <DialogActions><Button onClick={closeDialog} disabled={saving || removingId !== null || confirmingId !== null || discardOpen}>{text.close}</Button></DialogActions>
    <Dialog open={discardOpen} onClose={() => setDiscardOpen(false)} aria-labelledby="emergency-contact-discard-title" aria-describedby="emergency-contact-discard-description"><DialogTitle id="emergency-contact-discard-title">{text.discardTitle}</DialogTitle><DialogContent><Typography id="emergency-contact-discard-description">{text.discardBody}</Typography></DialogContent><DialogActions><Button autoFocus onClick={() => setDiscardOpen(false)}>{text.keepEditing}</Button><Button color="warning" variant="contained" onClick={discardDraftAndClose}>{text.discard}</Button></DialogActions></Dialog>
    <Dialog open={confirmingId !== null} onClose={closeRemoveConfirmation} fullWidth maxWidth="xs" aria-labelledby="emergency-contact-remove-title" aria-describedby="emergency-contact-remove-description"><DialogTitle id="emergency-contact-remove-title">{text.confirmTitle}</DialogTitle><DialogContent><Typography id="emergency-contact-remove-description">{text.confirmBody}</Typography></DialogContent><DialogActions><Button disabled={removingId !== null} onClick={closeRemoveConfirmation}>{text.cancel}</Button><Button color="error" variant="contained" disabled={removingId !== null} onClick={() => void remove()}>{removingId ? text.removing : text.confirm}</Button></DialogActions></Dialog>
  </Dialog>;
}
