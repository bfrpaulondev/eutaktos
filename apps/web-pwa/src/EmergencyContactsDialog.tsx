...
export function EmergencyContactsDialog({ personId, personName, locale, open, canWrite = false, onClose }: { personId: string; personName: string; locale: Locale; open: boolean; canWrite?: boolean; onClose(): void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setName(''); setPhone(''); setRelationship(''); setNotice('add');
      await load();
  ...
  return <>
    <Modal
      ...
      width={720}
      title={<Space direction="vertical" size={0}><Space wrap><Title level={4} style={{ margin: 0 }}>{text.title}</Title><Tag color="warning">{text.sensitive}</Tag></Space><Text type="secondary">{personName}</Text></Space>}
      footer={<Button onClick={closeDialog} disabled={saving || removingId !== null || confirmingId !== null || discardOpen}>{text.close}</Button>}
      styles={{ body: { maxHeight: '72vh', overflowY: 'auto' } }}
      destroyOnHidden
      aria-describedby="emergency-mode-description"
    >
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        ...
        {!canWrite ? <Alert type="info" showIcon title={text.readOnly} /> : null}
        {notice ? <Alert type="success" showIcon closable onClose={() => setNotice(null)} title={notice === 'add' ? text.successAdd : text.successRemove} /> : null}
        ...
        {loading ? <div role="status" aria-label={text.loading}><Skeleton active paragraph={{ rows: 3 }} /></div> : null}
        {!loading && !loadError && contacts.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={text.empty} /> : null}
        {!loading && !loadError ? contacts.map(contact => {
          const tel = safeTelHref(contact.phone);
          return <Card key={contact.id} size="small">
            <Space direction="vertical" size="small" style={{ width: '100%' }}>
              <Space align="start" style={{ width: '100%', justifyContent: 'space-between' }} wrap>
                <div><Text strong>{contact.name}</Text>{contact.relationship ? <><br /><Text type="secondary">{contact.relationship}</Text></> : null}</div>
                {canWrite ? <Button danger size="small" disabled={removingId !== null} onClick={event => { removeTriggerRef.current = event.currentTarget; setConfirmingId(contact.id); setOperationError(null); }}>{removingId === contact.id ? text.removing : text.remove}</Button> : null}
              </Space>
              <Text style={{ fontSize: 18, overflowWrap: 'anywhere' }}>{contact.phone}</Text>
              {tel ? <Button type="primary" href={tel} block>{text.call}</Button> : null}
            </Space>
          </Card>;
        }) : null}
        {canWrite ? <Card size="small" title={text.formTitle}>
          <form onSubmit={submit}>
            <Space direction="vertical" size="small" style={{ width: '100%' }}>
              <Input aria-label={text.name} placeholder={text.name} autoComplete="off" maxLength={120} value={name} onChange={event => setName(event.target.value)} required />
              <Input aria-label={text.phone} placeholder={text.phone} type="tel" inputMode="tel" autoComplete="off" maxLength={64} value={phone} onChange={event => setPhone(event.target.value)} required />
              <Input aria-label={text.relationship} placeholder={text.relationship} autoComplete="off" maxLength={120} value={relationship} onChange={event => setRelationship(event.target.value)} />
              <Button htmlType="submit" type="primary" loading={saving} disabled={!canSubmitEmergencyContact(name, phone, saving)} block>{saving ? text.adding : text.add}</Button>
            </Space>
          </form>
        </Card> : null}
      </Space>
