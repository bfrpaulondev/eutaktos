...
export function hasUnsavedEmergencyContactDraft(name: string, phone: string, relationship: string): boolean { return Boolean(name.trim() || phone.trim() || relationship.trim()); }
export function emergencyContactAccess(capabilities: readonly string[]): Readonly<{ canRead: boolean; canWrite: boolean }> { const canRead = capabilities.includes('emergency-contacts.read'); return Object.freeze({ canRead, canWrite: canRead && capabilities.includes('emergency-contacts.write') }); }
function safeTelHref(phone: string): string | undefined { const normalized = phone.replace(/[^+0-9]/g, ''); return normalized ? `tel:${normalized}` : undefined; }
...
export function EmergencyContactsDialog({ personId, personName, locale, open, onClose }: { personId: string; personName: string; locale: Locale; open: boolean; onClose(): void }) {
  ...
  const loadContacts = async (signal?: AbortSignal) => {
    const request = ++requestRef.current; setLoading(true); setLoadError(false);
    try { const value = await emergencyContactsApi.list(personId, signal); if (signal?.aborted || request !== requestRef.current) return; setContacts(value); }
    catch (reason) { if (signal?.aborted || request !== requestRef.current || (reason instanceof DOMException && reason.name === 'AbortError')) return; setLoadError(true); }
  ...
  const initialize = async (signal?: AbortSignal) => {
    ...
    try {
      const session = await sessionApi.current(signal); if (signal?.aborted) return;
      const access = emergencyContactAccess(session.capabilities);
      if (!access.canRead) { setAccessState('forbidden'); return; }
      setCanWrite(access.canWrite); setAccessState('ready'); await loadContacts(signal);
    } catch (reason) { if (signal?.aborted || (reason instanceof DOMException && reason.name === 'AbortError')) return; setAccessState('error'); }
  };

  useEffect(() => { if (!open) return; const controller = new AbortController(); void initialize(controller.signal); return () => { requestRef.current += 1; controller.abort(); }; }, [open, personId]);

  const submit = async (event: FormEvent) => {
    event.preventDefault(); if (!canWrite || !canSubmitEmergencyContact(name, phone, saving) || savingRef.current) return;
    savingRef.current = true; setSaving(true); setOperationError(null); setNotice(null);
    try { await emergencyContactsApi.create(personId, { name: name.trim(), phone: phone.trim(), relationship: relationship.trim() || undefined }); setName(''); setPhone(''); setRelationship(''); setNotice('add'); await loadContacts(); }
    catch { setOperationError('save'); } finally { savingRef.current = false; setSaving(false); }
  };

  const remove = async () => {
    if (!canWrite || !confirmingId || removingRef.current) return; const id = confirmingId;
    removingRef.current = true; setRemovingId(id); setOperationError(null); setNotice(null);
    try { await emergencyContactsApi.remove(personId, id); setConfirmingId(null); setNotice('remove'); await loadContacts(); window.requestAnimationFrame(() => removeTriggerRef.current?.focus()); }
  ...
  const closeDialog = () => { if (saving || removingId || confirmingId) return; if (canWrite && hasUnsavedEmergencyContactDraft(name, phone, relationship)) { setDiscardOpen(true); return; } onClose(); };
  ...
  return <>
    <Modal open={open} onCancel={closeDialog} width={720} title={<Space direction="vertical" size={0}><Space wrap><Title level={4} style={{ margin: 0 }}>{text.title}</Title><Tag color="warning">{text.sensitive}</Tag></Space><Text type="secondary">{personName}</Text></Space>} footer={<Button onClick={closeDialog} disabled={saving || removingId !== null || confirmingId !== null || discardOpen}>{text.close}</Button>} styles={{ body: { maxHeight: '72vh', overflowY: 'auto' } }} destroyOnHidden aria-describedby="emergency-mode-description">
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        <Paragraph id="emergency-mode-description" type="secondary" style={{ marginBottom: 0 }}>{text.subtitle}</Paragraph><Alert type="warning" showIcon title={text.restricted} />
        ...
        {accessState === 'ready' && !loading && !loadError ? contacts.map(contact => { const tel = safeTelHref(contact.phone); return <Card key={contact.id} size="small"><Space direction="vertical" size="small" style={{ width: '100%' }}><Space align="start" style={{ width: '100%', justifyContent: 'space-between' }} wrap><div><Text strong>{contact.name}</Text>{contact.relationship ? <><br /><Text type="secondary">{contact.relationship}</Text></> : null}</div>{canWrite ? <Button danger size="small" disabled={removingId !== null} onClick={event => { removeTriggerRef.current = event.currentTarget; setConfirmingId(contact.id); setOperationError(null); }}>{removingId === contact.id ? text.removing : text.remove}</Button> : null}</Space><Text style={{ fontSize: 18, overflowWrap: 'anywhere' }}>{contact.phone}</Text>{tel ? <Button type="primary" href={tel} block>{text.call}</Button> : null}</Space></Card>; }) : null}
