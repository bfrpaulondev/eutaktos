  ...
  return <Box component="section" aria-labelledby={`midweek-${section}-title`}>
    <Paper sx={{ p: { xs: 2, sm: 3 }, mb: 2, borderRadius: 3 }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" gap={2} alignItems={{ sm: 'flex-start' }}>
        ...
        {!loading && !loadError && overview ? <Chip label={section === 'agenda' ? String(overview.meetings.length) : String(assignmentCount)} variant="outlined" color="info" /> : null}
      </Stack>
    </Paper>
    ...
    {!loading && !loadError && overview && section === 'agenda' && overview.meetings.length === 0 ? <Paper variant="outlined" sx={{ p: 5, textAlign: 'center', borderRadius: 3, boxShadow: 'none', bgcolor: 'transparent' }}><Typography color="text.secondary">{text.emptyAgenda}</Typography></Paper> : null}
    ...
    {!loading && !loadError && overview && section === 'assignments' && assignmentCount > 0 ? <Stack spacing={1.25}>
      {overview.studentAssignments.map(assignment => { const meeting = meetingsById.get(assignment.meetingId); return <Card component="article" key={`student-${assignment.id}`}><CardContent><Stack spacing={1.25}>
        ...
        {meeting ? <Typography variant="body2" color="text.secondary">{text.meeting}: {dateLabel(meeting, locale)}</Typography> : null}
