...
export function PersonWizardOrganizationStep({ draft, households, groups, state, canWrite, labels, onChange, onRetry }: { draft: PersonWizardDraft; households: readonly HouseholdDto[]; groups: readonly ServiceGroupDto[]; state: 'loading' | 'ready' | 'error' | 'forbidden'; canWrite: boolean; labels: Readonly<{ loading: string; error: string; forbidden: string; retry: string; empty: string; households: string; groups: string; optional: string; noWrite: string }>; onChange: (change: Partial<PersonWizardDraft>) => void; onRetry: () => void }) {
