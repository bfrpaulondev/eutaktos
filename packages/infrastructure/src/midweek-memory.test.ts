...
const context = (tenantId: string, capabilities = ['schedule.write'] as const) =>
...
describe('InMemoryMidweekSchedulingUnitOfWork', () => {
  ...
  it('stores audit and outbox events only in their tenant', () => {
    ...
    const auditReader = context('a', ['audit.read'] as const);
    const outboxReader = context('a', ['tenant.manage'] as const);
    expect(uow.listAudit(auditReader)).toHaveLength(1);
    expect(uow.listOutbox(outboxReader)).toHaveLength(1);
    expect(uow.listAudit(context('b', ['audit.read'] as const))).toHaveLength(0);
  });
