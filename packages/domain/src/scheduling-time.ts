export type ZonedLocalTimeResolution = Readonly<{
  instant: string;
  ambiguous: boolean;
}>;
...
function offsetMinutes(epochMs: number, timezone: string): number {
  ...
  return Math.round((asUtc - Math.floor(epochMs / 1000) * 1000) / 60000) + Math.floor((epochMs % 60000) / 60000);
}
...
export function resolveZonedLocalTime(dateInput: string, localTimeInput: string, timezoneInput: string): ZonedLocalTimeResolution {
  ...
  for (let deltaHours = -48; deltaHours <= 48; deltaHours += 6) {
    ...
    const offset = offsetMinutes(sample, timezone);
    candidateEpochs.add(baseUtc - offset * 60_000);
  }

  const matches = [...candidateEpochs]
    .filter(epochMs => localParts(epochMs, timezone) === target)
    .sort((a, b) => a - b);
