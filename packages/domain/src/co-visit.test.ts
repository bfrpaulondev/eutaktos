...
describe('orderAgendaSlots', () => {
  it('sorts by startsAt', () => {
    const v = make({ agendaSlots: [SLOT_B, SLOT_A] });
    const ordered = orderAgendaSlots(v);
    expect(ordered[0].title).toBe('Public Talk');
  });
});

describe('validateAgendaWithinVisit', () => {
  it('passes for valid', () => { expect(() => validateAgendaWithinVisit(make())).not.toThrow(); });
  it('throws when slot starts before visit', () => {
    const v = make({ agendaSlots: [{ title: 'Early', startsAt: '2026-09-05T13:00:00Z', endsAt: '2026-09-05T14:00:00Z' }] });
    expect(() => validateAgendaWithinVisit(v)).toThrow('starts before the visit');
  });
  it('throws when slot ends after visit', () => {
    const v = make({ agendaSlots: [{ title: 'Late', startsAt: '2026-09-05T15:00:00Z', endsAt: '2026-09-05T17:00:00Z' }] });
    expect(() => validateAgendaWithinVisit(v)).toThrow('ends after the visit');
  });
});
