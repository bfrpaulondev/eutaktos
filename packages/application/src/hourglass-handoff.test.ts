...
const person = { id: 'person-1', tenantId: 'tenant-a', displayName: 'Ana Exemplo', externalHourglassPersonId: 'hourglass:publisher:101' };

describe('Hourglass handoff', () => {
  it('creates a manual-only handoff from real Eutaktos assignment shapes', () => {
    const handoff = createHourglassHandoff({ tenantId: 'tenant-a', meetings: [meeting], studentAssignments: [assignment], nonStudentAssignments: [], people: [person] });
  ...
  it('does not mix meetings, assignments or people from another tenant', () => {
    ...
    expect(() => createHourglassHandoff({ tenantId: 'tenant-a', meetings: [meeting], studentAssignments: [{ ...assignment, tenantId: 'tenant-b' }], nonStudentAssignments: [], people: [person] })).toThrow('Cross-tenant handoff access denied');
    expect(() => createHourglassHandoff({ tenantId: 'tenant-a', meetings: [meeting], studentAssignments: [assignment], nonStudentAssignments: [], people: [{ ...person, tenantId: 'tenant-b' }] })).toThrow('Cross-tenant handoff access denied');
  });
});
