...
const handler: ApiHandler = async (request, response) => {
  if (!['GET','PUT','PATCH'].includes(request.method ?? '')) { methodNotAllowed(response, ['GET','PUT','PATCH']); return; }
  await runEndpoint(request, response, async database => {
    ...
    const transport = new PeopleHttpTransport(service);
    const transportReq = transportRequest(request, principal, { personId });
    const result = request.method === 'GET' ? transport.get(transportReq) : (() => { assertTrustedMutation(request); return transport.update(transportReq); })();
    if (request.method !== 'GET' && result.status >= 200 && result.status < 300) await unitOfWork.flush(database);
