...
function unavailable(): EvidenceUnavailable { return Object.freeze({ status: 'unavailable' }); }
function instant(value: string, field: string): number { const parsed = Date.parse(value); if (!Number.isFinite(parsed)) throw new Error(`Invalid ${field}`); return parsed; }

function availabilityFor(person: Readonly<CongregationPerson>, nowMs: number): AvailabilityReady {
  const current = person.availability.filter(period => instant(period.startsAt, 'availability startsAt') <= nowMs && nowMs < instant(period.endsAt, 'availability endsAt'));
  const next = person.availability.filter(period => instant(period.startsAt, 'availability startsAt') > nowMs).sort((left, right) => instant(left.startsAt, 'availability startsAt') - instant(right.startsAt, 'availability startsAt'))[0];
  ...
  return Object.freeze({ status: 'ready', current: current.length ? 'unavailable' : 'available', currentReasonCodes: Object.freeze(reasonCodes), ...(next ? { nextPeriod: Object.freeze({ startsAt: next.startsAt, endsAt: next.endsAt, ...(next.reasonCode ? { reasonCode: next.reasonCode } : {}) }) } : {}) });
}
...
function eligibilityFor(person: Readonly<CongregationPerson>): EligibilityReady {
  ...
  for (const decision of person.eligibility) {
    ...
    if (!existing || instant(existing.decidedAt, 'eligibility decidedAt') <= instant(decision.decidedAt, 'eligibility decidedAt')) latest.set(decision.assignmentTypeId, decision);
  }
  return Object.freeze({ status: 'ready', enabledAssignmentTypeIds: Object.freeze([...latest.values()].filter(decision => decision.enabled).map(decision => decision.assignmentTypeId).sort()) });
}

function activeResponsibilitiesFor(personId: string, responsibilities: readonly Readonly<ResponsibilityAssignment>[], nowIso: string): ResponsibilitiesReady {
  return Object.freeze({ status: 'ready', keys: Object.freeze(responsibilities.filter(item => item.personId === personId && isResponsibilityActiveAt(item, nowIso)).map(item => item.responsibilityKey).sort()) });
}
...
const handler: ApiHandler = async (request, response) => {
  ...
  await runEndpoint(request, response, async database => {
    ...
    if (canReadSchedule) {
      ...
      for (const item of history) { const previous = historyByPersonId.get(item.personId); if (!previous || previous < item.meetingDate) historyByPersonId.set(item.personId, item.meetingDate); }
    }

    const now = new Date();
    const nowIso = now.toISOString();
    const peopleProjection = people.map(person => {
      const groupMemberships = groups.filter(group => group.memberIds.includes(person.id)).map(group => Object.freeze({ id: group.id, name: group.name })).sort((left, right) => left.name.localeCompare(right.name) || left.id.localeCompare(right.id));
      ...
      return Object.freeze({
        ...
        assignmentHistory: canReadSchedule ? Object.freeze({ status: 'ready' as const, ...(lastCompletedMeetingDate ? { lastCompletedMeetingDate } : {}) }) : unavailable(),
      });
    }).sort((left, right) => left.displayName.localeCompare(right.displayName) || left.id.localeCompare(right.id));

    const responsibilityKeys = canReadResponsibilities ? [...new Set(responsibilities.filter(item => isResponsibilityActiveAt(item, nowIso)).map(item => item.responsibilityKey))].sort() : [];
    const assignmentTypeIds = canReadEligibility ? [...new Set(people.flatMap(person => eligibilityFor(person).enabledAssignmentTypeIds))].sort() : [];
    const labels = [...new Set(people.flatMap(person => person.labels ?? []))].sort((left, right) => left.localeCompare(right));

    json(response, 200, Object.freeze({
      contractVersion: 'people-directory-v1', generatedAt: nowIso,
      capabilities: Object.freeze({ writePeople: canWritePeople, availability: canReadAvailability, eligibility: canReadEligibility, responsibilities: canReadResponsibilities, schedule: canReadSchedule }),
